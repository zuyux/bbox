    ...
    try {
      ...
      for (const derived of candidates) {
        ...
        if (response.ok && result.exists) {
          setWallet({ ...derived, label: result.walletLabel || 'BBOXX Wallet' });
          setStep('password');
          return;
        }
      }

      throw new Error(translate('No BBOXX account was found for this recovery phrase.'));
    } catch (cause) {
    ...
    <div className="space-y-4 text-foreground">
      ...
      {step === 'phrase' ? (
        <>
          ...
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-300">{translate('Never share this phrase. BBOXX support will never ask for it.')}</div>
